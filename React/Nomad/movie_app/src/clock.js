...
function getTime() {
  ...
  clock.innerText = `${monthsToDate() + setMonth(LeftDay)}d ${setHour(
    LeftHours
  )}h ${setTime(LeftMinutes)}m ${setTime(LeftSeconds)}s`;
}
...
function getTime() {
 const today = new Date();
  clock.innerText = `${set(today.getHours())}:${set(today.getMinutes())}:${set(today.getSeconds())}`;
}
